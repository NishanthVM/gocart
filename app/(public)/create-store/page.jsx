    ...
    try {
      ...
      if (["approved", "pending", "rejected"].includes(data.status)) {
        ...
        switch (data.status) {
          case "approved":
            setMessage(
              "Your store is approved! You can now add products from store dashboard."
            );
            setTimeout(() => {
              router.push("/store");
            }, 5000);
          ...
          case "rejected":
            setMessage(
              "Your store is rejected! Please contact support for more information."
            );
            break;
          case "pending":
            setMessage(
              "Your store is pending approval. You will be notified once it's reviewed."
            );
